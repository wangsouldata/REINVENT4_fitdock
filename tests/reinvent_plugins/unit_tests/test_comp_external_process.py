...
""")
    ...
    # Setup component
    params = Parameters(executable=[sys.executable], args=[script], property=["predictions"])
    ...
    smiles_results = SmilesAssociatedComponentResults(component_results=results, smiles=smiles)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"predictions": [0.5, 0.6]}, ["predictions"], [[0.5, 0.6]], []),
        # With metadata
        (
...
"""
    ...
    params = Parameters(executable=[sys.executable], args=[script], property=property_names)
    component = ExternalProcess(params)
    ...
    assert len(smiles_results.data) == len(
        smiles
